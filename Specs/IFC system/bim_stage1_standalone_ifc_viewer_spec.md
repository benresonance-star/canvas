...
| Color-by IFC type | Yes — toolbar `Palette` toggle; `displayMode: colorBy` + `colorByProperty: ifcClass`; palette kept ≥0.2 RGB from selection orange; **IFC class filter** scopes viewport/table colours; **stable tints during zoom/orbit** (render-only loop); element-table swatches; BQL `colorBy` supported (`bimColorBy.js`) |
| Geometry repair / submesh edit | **Partial** — toolbar wrench HUD (`BimGeometryRepairToolbarControls.jsx`); shell load + coloured overlay (`bimGeometryRepair.js`, `bimGeometryRepairOverlay.js`); flip-normals per shell persisted in `geometryRepairs`; **reliable in clay mode**; highlight/ghost viewport consistency and normal-flip verification **still need attention** (master spec §18.11.2) |
...
| Element table | Yes — configurable columns (add/remove/reorder/resize/sort), search + class filter, **floating left overlay panel**, header sort menu anchored to column title, inset grid dividers; **IFC-class colour swatches** when colour-by active (respects class filter); (`bimTableColumns.js`, `bimColorBy.js`) |
...
| Workspace state persistence | Yes — IndexedDB per fingerprint (camera, panels, filters, display mode, **colorByProperty**, projection, measurements, wireframe mode + style incl. hidden-lines toggle, clay render style, lighting, **geometry repairs** (`geometryRepairs`), **table columns/sort/search**, **inspector search**, **panel widths**) |
