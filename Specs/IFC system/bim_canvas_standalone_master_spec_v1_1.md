...
- **Federated IFC viewer session (2026-07-10):** Add menu → **IFC Viewer** creates an empty `bim-model` card with `version.bim.viewerKind: 'ifc-viewer-session'` and an empty `version.bim.session` (`modelRefs`, `activeModelId`, `visibilityByModelId`, `queryScope: 'active'`). Users import one or more `.ifc` files into the session via the toolbar **File** HUD; each import runs `prepareBimModel()` and stores a `modelRef` (`modelId`, `label`, `fingerprint`, `status`). `activeModelId` selects which prepared model drives the viewport and element table; `visibilityByModelId` tracks per-file show/hide (UI state; federated scene composition deferred). Session patches persist via `patchVersionBim` on the card version. Header strip shows active file + loaded/visible counts when models are present.
...
| Federated IFC viewer session | **Partial (MVP)** — empty session card + multi-file import + active-file switching + per-file visibility UI; single active model in viewport (not simultaneous federated scene composition) |
