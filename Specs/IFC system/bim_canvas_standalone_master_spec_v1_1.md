...
| Federated IFC viewer session | **Partial (MVP)** — empty session card + multi-file import + active-file switching + **simultaneous multi-model viewport** with per-file visibility; selection/table/inspector scoped to active file; linked-folder source refresh deferred |
