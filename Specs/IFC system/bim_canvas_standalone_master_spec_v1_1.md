...
**Single selection (shipped, 2026-07-09):** only one physical element is selected at a time in the viewer/table/inspector loop. Highlight mode clears previous fragment highlights before applying the new selection (`model.resetHighlight()`), then forces a fragments refresh (`updateFragments(true)`). `applySelection` uses run-sequence cancellation after every async step so stale runs cannot re-highlight a superseded element; a deferred resync self-heals if a stale run completes a highlight after being cancelled.
...
## 18.10 Zoom to selection (shipped MVP, 2026-07-09)

Optional **Focus** toggle in the **view group**, immediately **right of isolate** (left of FOV / camera controls).

- **Workspace field:** `zoomToSelectionOnSelect` (default `false`; persisted like `isolateOnSelect`; included in saved view state via `bimViewSets.js`).
- **When off:** selection highlight/isolate behaviour unchanged; camera stays put.
- **When on:** each new selection (viewport pick or element-table row) animates the camera to frame the selected element(s). Query/batch highlights frame the active result set. Uses fragment geometry bounds from `getItemsGeometry()` (`bimSelectionCameraFit.js`), fits from the **current view direction** (`fitPerspectiveCameraToCurrentView` / `fitOrthographicCameraToCurrentView` in `cameraFit.js`), and animates via `createBimCameraHistoryAnimator()` (~400 ms). Records one camera-history undo step per zoom.
- **Enabling while selected:** toggling on with an active selection zooms immediately.

**Deferred:** per-element framing presets (home vs current view toggle); zoom on deselect reset.

## 18.11 Geometry repair / submesh edit (partial MVP, 2026-07-10)

Toolbar **wrench** icon in the **tools group** opens a portalled **Geometry repair** HUD (`BimGeometryRepairToolbarControls.jsx`). Requires a selected element.

### 18.11.1 Shipped behaviour

- **Edit geometry toggle:** when on and an element is selected, loads mesh **shells** (submeshes) for the active element via Fragments geometry APIs with fallbacks (`getItemsGeometry`, `getItemsWithGeometry` / `getItem`, scene extraction by `userData.itemId`, federated runtime model scan).
- **Submesh HUD:** lists **Shell 1…N** buttons; viewport pick in edit mode selects a shell for **Flip normals** (per-shell triangle winding toggle).
- **Viewport overlay:** coloured semi-transparent shell overlay on the wireframe overlay scene (`bimGeometryRepairOverlay.js`); selected shell reads orange (`#f59e0b`); edit mode uses distinct submesh palette.
- **Hide base geometry:** while editing, resolved fragment local IDs for the element are hidden via `model.setVisible(ids, false)` so the overlay replaces the source mesh (restored on teardown).
- **Pick context:** viewport raycast stores `{ elementId, localIds, itemIds }` on pick to improve proxy / `IfcBuildingElementProxy` geometry resolution (e.g. imported mesh cars with geometry on non-proxy local IDs).
- **Persistence:** flipped shell indexes per element in workspace state (`geometryRepairs[elementId].flippedShellIndexes`); normalised via `normalizeGeometryRepairs()` in `types.js`.
- **Clay mode:** **reliable** end-to-end flow — select element → enable **Edit geometry** → submeshes populate HUD and viewport overlay.
- **Display-mode coordination:** edit mode runs a dedicated `applySelection` path — sync repair overlay first, then optional `fragments.update()`, then re-sync; ghost/highlight/isolate styling applies to non-edited geometry without re-highlighting the edited element.

### 18.11.2 Known gaps — **needs further attention**

> **Flag (2026-07-10):** Geometry repair interaction and function still need work before treating this feature as complete.

| Area | Status |
|---|---|
| **Viewport display consistency** | Submesh overlay and HUD population are **consistent in clay mode** but remain **inconsistent in highlight and ghost modes** on first enable (may require visiting clay mode or extra mode switches). Display-mode transitions (ghost ↔ highlight) while edit geometry is on can leave stale ghost/highlight state or drop the overlay. |
| **Normal flip success** | Flip-normals toggles persist per shell index, but **visual confirmation of flipped winding** is not always reliable across render styles / display modes; verify against ground truth on complex imported meshes. |
| **Proxy / federated geometry** | Multi-shell resolution for `IfcBuildingElementProxy` and federated sessions depends on pick context and geometry-item scans — retest across files not loaded as the active model. |
| **Worker / sync races** | Multiple concurrent geometry-repair sync paths and `fragments.update()` can still abort or reorder hide/show; further consolidation recommended. |

**Deferred:** apply flipped winding back to Fragments source meshes (overlay-only flip today); bulk flip; shell merge/split; geometry repair in color-by mode; server-side persistence beyond workspace state.
...
canvas/src/features/bim/
  bim-core/
    ...
    bimGeometryRepair.js     # mesh shell loading, geometry ID resolution, hide/show for edit mode
    bimGeometryRepairOverlay.js # coloured submesh overlay + pick + render pass
    ...
    bimSelectionCameraFit.js # zoom-to-selection bounds + camera fit from current view
  ...
  components/
    BimWorkspace.jsx
    BimViewport.jsx
    BimFileToolbarControls.jsx  # federated-session file manager toolbar HUD
    ...
    BimGeometryRepairToolbarControls.jsx  # geometry repair / submesh edit HUD (wrench icon, tools group)
...
- **Federated IFC viewer session (2026-07-10):** Add menu → **IFC Viewer** creates an empty `bim-model` card with `version.bim.viewerKind: 'ifc-viewer-session'` and an empty `version.bim.session` (`modelRefs`, `activeModelId`, `visibilityByModelId`, `queryScope: 'active'`). Users import one or more `.ifc` files into the session via the toolbar **File** HUD; each import runs `prepareBimModel()` and stores a `modelRef` (`modelId`, `label`, `fingerprint`, `status`, `sourceStorageKey`). Uploaded IFC blobs are cached in IndexedDB `modelSources` (`bimRepository.js` DB v3). `activeModelId` selects which prepared model drives selection, element table, and inspector; **all visible models** load into the viewport simultaneously via `BimViewport` runtime model map (`preparedModels`, `visibilityByModelId`). Per-file visibility toggles show/hide geometry in the 3D scene and recompute viewport bounds from visible roots. Session patches persist via `patchVersionBim` on the card version. Header strip shows active file + loaded/visible counts when models are present. Card preview (`BimModelSummary`) lists loaded file names when present.
- **IFC Viewer graph linking (2026-07-11):** Federated viewer session cards are canvas-native (not folder-backed) but are linkable on the project graph like folder-ingested BIM models. On create (`handleSaveNewBimViewer`) and on first link attempt (`ensureCardArtifactRef`), `registerBimViewerSessionArtifact` ingests a primitives `bim_model` artifact with URI `canvas-bim-viewer:{projectId}/{cardId}`, stable content hash from session identity, and metadata `canvas_kind: 'bim-viewer-session'`. The pinned version receives `artifactRef` so drag-to-link and new-task link targets succeed without requiring a connected folder. `NewTaskDialog` lists `bim-viewers` cards even when refs are not yet materialized; `handleSaveNewTask` resolves `linkTargetCards` before `finalizeArtifactLinks`.
- **Federated session default layout (2026-07-10):** `applyBimSessionViewerDefaults()` in `types.js` collapses left/right floating side panels on workspace open (`panels: { left: false, right: false }`) so the federated viewer starts with maximum viewport area; users expand panels via toolbar toggles.
...
- **Floating side panels (2026-07-06, updated 2026-07-10):** toolbar toggles open **left** (element table) and **right** (inspector) overlay panels (`BimFloatingSidePanel.jsx`) over the viewport — same visual language as other HUDs (`rounded-md`, `bg-surface/95`, shadow). Width 240–720 px with invisible inner-edge resize handles; persisted in `leftPanelWidth` / `rightPanelWidth`. Panels and tool HUD stacks start below the toolbar band (`top-14`); toggling panels does not reflow the canvas or toolbar. **Federated IFC viewer sessions** start with both panels **collapsed** (`applyBimSessionViewerDefaults`).
...
- **Geometry repair / submesh edit (partial MVP)** (2026-07-10): toolbar **wrench** icon in the tools group opens `BimGeometryRepairToolbarControls.jsx` — **Edit geometry** toggle, per-shell HUD list, **Flip normals** per selected shell. Loads mesh shells via `bimGeometryRepair.js` (Fragments geometry APIs + scene fallbacks + federated model scan); coloured overlay on wireframe overlay scene (`bimGeometryRepairOverlay.js`); hides base fragment geometry while editing; pick context stores `{ elementId, localIds, itemIds }` for proxy geometry resolution. **Clay mode:** reliable select → edit → submesh HUD + viewport overlay. **Known gaps (needs attention):** viewport display consistency across **highlight** and **ghost** modes on first enable; ghost ↔ highlight transitions while edit geometry is on; **normal flip** visual confirmation not always reliable — see §18.11.2.
- Workspace state (camera, selection, filters, display mode, projection mode, measurements, **`rlDatum`**, `measureKind` (`segment` | `polyline` | `rl` | `datum`), wireframe mode + style (`wireframeHiddenLines` included), **render style + clay tuning**, **section cut state**, **environmentalAnalysis (sun study)**, **view sets + active view/set + carousel open**, **saved queries**, **saved result sets**, **4D sequences**, **5D cost plans**, **geometry repairs** (`geometryRepairs` per-element flipped shell indexes), lighting, panel toggles, **element table columns/sort/search/filters**, **inspector attribute search**, **left/right panel widths**) persisted in IndexedDB per fingerprint.
...
| Federated IFC viewer session | **Partial (MVP)** — empty session card + multi-file import + active-file switching + **simultaneous multi-model viewport** with per-file visibility; selection/table/inspector scoped to active file; **graph linking** via canvas-native primitives ingest (`bimViewerArtifact.js`); linked-folder source refresh deferred |
| File manager toolbar HUD | **Shipped (MVP)** — `File` icon far left of viewport toolbar; portalled panel below icon (`BimFileToolbarControls.jsx`); toggle open/close; import + file list + visibility/remove |
| Geometry repair / submesh edit | **Partial (MVP)** — wrench HUD + shell overlay + flip-normals persistence; **reliable in clay mode**; **viewport display consistency** (highlight/ghost) and **normal flip success** still need attention (§18.11.2) |
