...
export function buildArchitectureMarkdown(runtime) {
  const lines = [
    ...
    '- **Action sync:** pointer-up layout commit, structural changes, project switch, folder scan, visibility resume (no ~3s project poll)',
    '- Debounced saves (~600ms) use `pushProjectDocumentIfLocalNewer` (CAS with `expectedRevision` → 409 on conflict)',
